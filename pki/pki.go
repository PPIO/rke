	...
	for _, node := range rkeConfig.Nodes {
		if node.Address == nodeAddress {
			...
			if IsKubeletEnableNodeAuthorization(&rkeConfig) {
				var replaceCert CertificatePKI
				nodeKey := GetCrtNameForHost(&hosts.Host{RKEConfigNode: node}, KubeNodeCertName)
				replaceCert = certBundle[nodeKey]
				certBundle[KubeNodeCertName] = replaceCert
				log.Infof(ctx, "[certificates] overwrite kube node cert for node %s, %v", nodeKey, replaceCert)
				break
			}
